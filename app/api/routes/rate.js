// Express router version of rate.ts
import express, { Request, Response } from 'express';

const router = express.Router();

let cachedRate = null;
let cacheTimestamp = null;
const CACHE_DURATION_MS = 60 * 60 * 1000; // 1 hour
const FALLBACK_RATE = 1500;

router.get('/', async (req: Request, res: Response) => {
  try {
    const now = Date.now();

    // Return cached rate if still valid (within 1 hour)
    if (cachedRate && cacheTimestamp && (now - cacheTimestamp) < CACHE_DURATION_MS) {
      return res.json({
        rate: cachedRate,
        cached: true,
        updatedAt: new Date(cacheTimestamp).toISOString(),
      });
    }

    // Fetch real-time rate using API key from environment variable
    const apiKey = process.env.VITE_EXCHANGE_RATE_API_KEY || process.env.EXCHANGE_RATE_API_KEY;

    if (!apiKey) {
      // No API key configured — use fallback
      cachedRate = FALLBACK_RATE;
      cacheTimestamp = now;
      return res.json({
        rate: FALLBACK_RATE,
        cached: false,
        fallback: true,
        error: 'Missing exchange rate API key',
        updatedAt: new Date(cacheTimestamp).toISOString(),
      });
    }

    const response = await fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/latest/USD`);

    if (!response.ok) {
      throw new Error(`Exchange rate API returned ${response.status}`);
    }

    const data = await response.json();

    if (data && data.conversion_rates && data.conversion_rates.NGN) {
      cachedRate = data.conversion_rates.NGN;
      cacheTimestamp = now;

      return res.json({
        rate: cachedRate,
        cached: false,
        updatedAt: new Date(cacheTimestamp).toISOString(),
      });
    } else {
      // API response missing NGN rate — use fallback
      cachedRate = FALLBACK_RATE;
      cacheTimestamp = now;

      return res.json({
        rate: FALLBACK_RATE,
        cached: false,
        fallback: true,
        error: 'NGN rate not found in API response',
        updatedAt: new Date(cacheTimestamp).toISOString(),
      });
    }
  } catch (error: any) {
    // Network or parsing error — use fallback
    cachedRate = FALLBACK_RATE;
    cacheTimestamp = Date.now();

    return res.json({
      rate: FALLBACK_RATE,
      cached: false,
      fallback: true,
      error: error.message || 'Failed to fetch exchange rate',
      updatedAt: new Date(cacheTimestamp).toISOString(),
    });
  }
});

export default router;