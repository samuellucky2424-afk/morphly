...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (data.status && data.data.status === 'success') {
      ...
      let { data: walletData } = await supabaseAdmin.from('wallets').select('balance').eq('user_id', userId).single();
      ...
      await supabaseAdmin.from('transactions').insert({
        user_id: userId, type: 'credit', amount: amountInNaira, reference: reference, status: 'success', created_at: new Date()
      });

      const pName = planName || 'Custom';
      const pMins = planMinutes || Math.round(amountInNaira / 69.2 / 60);

      await supabaseAdmin.from('subscriptions').insert({
        user_id: userId, plan_name: pName, amount_paid: amountInNaira, credits: pMins, status: 'active', created_at: new Date()
      });
        
      res.json({ status: 'success', message: 'Payment verification successful', data: data.data, newBalance });
    } else {
