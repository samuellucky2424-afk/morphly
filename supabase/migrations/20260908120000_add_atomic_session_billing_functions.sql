-- =============================================================================
-- ADD MISSING ATOMIC SESSION BILLING FUNCTIONS (production repair)
--
-- The live API calls public.record_ai_session_usage and public.finalize_ai_session
-- on every heartbeat and session end, but they were never applied to production
-- (every call currently returns 404 and falls back to slower legacy paths).
-- This migration is idempotent: it is safe to run more than once and it does
-- not alter historical wallet balances or existing session rows.
--
-- Apply in the Supabase SQL Editor, then press "Run". No app update required.
-- =============================================================================

-- Columns the functions (and the current API fallbacks) rely on.
...
-- Durable ledger the functions write into (no-op when it already exists).
...
CREATE UNIQUE INDEX IF NOT EXISTS wallet_ledger_idempotency_key_unique
