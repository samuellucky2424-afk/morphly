...
namespace morphly
{
    ...
    bool Publisher::IsMediaFoundationReceiverActive(uint64_t maximumAgeMilliseconds) const noexcept
    {
        const auto readHeartbeat = [](const void* view) noexcept -> uint64_t
        {
            if (view == nullptr)
            {
                return 0;
            ...
            const auto* header = static_cast<const SharedFrameHeader*>(view);
            if (header->magic != kProtocolMagic || header->version != kProtocolVersion)
            {
                return 0;
            }

            auto* heartbeat = reinterpret_cast<volatile LONG64*>(
                const_cast<uint64_t*>(&header->consumerHeartbeatTickMs));
            return static_cast<uint64_t>(InterlockedCompareExchange64(heartbeat, 0, 0));
        };

        const uint64_t heartbeat = std::max(
            readHeartbeat(mfBridgeView_),
            std::max(readHeartbeat(globalView_), readHeartbeat(view_)));
        if (heartbeat == 0)
        {
            return false;
        }
        ...
        return now >= heartbeat && (now - heartbeat) <= maximumAgeMilliseconds;
    }
